...
from model.faster_rcnn.light_head import LightHead
...
if __name__ == '__main__':
    ...
    for batch_idx in range(num_images // args.batch_size):
        data = next(data_iter)
        ...
        for offset in range(args.batch_size):
            im_scores = scores[offset]
            im_pred_boxes = pred_boxes[offset]

            im_pred_boxes /= im_info[offset][2].data

            i = batch_idx * args.batch_size + offset

            if vis:
                im = cv2.imread(imdb.image_path_at(i))
                im2show = np.copy(im)
            for j in xrange(1, imdb.num_classes):
                inds = torch.nonzero(im_scores[:, j] > thresh).view(-1)
                # if there is det
                if inds.numel() > 0:
                    cls_scores = im_scores[:, j][inds]
                    _, order = torch.sort(cls_scores, 0, True)

                    if args.class_agnostic:
                        cls_boxes = im_pred_boxes[inds, :]
                    else:
                        cls_boxes = im_pred_boxes[inds][:, j * 4:(j + 1) * 4]
        ...
        sys.stdout.write('im_detect: {:d}/{:d} {:.3f}s {:.3f}s   \r' \
                         .format(i + 1, num_images, detect_time/args.batch_size, nms_time))
        sys.stdout.flush()
